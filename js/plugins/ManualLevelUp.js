...
(() => {
  ...
  const helps = [
    'レベル:レベルを上げることでHPやSPが上昇し、\nパラメータポイントを入手します。',
    '筋力:[筋力判定]の武器や、技能の威力に影響。\n脱出判定の成功率に影響。HPの最大値に影響。',
    '技量:[技量判定]の武器や、技能の威力に影響。\n回避、逃走、解錠判定の成功率に影響。SPの最大値に影響。',
    '精神:[精神判定]の武器や、技能の威力に影響。\n抵抗判定の成功率に影響。HPとSPの最大値に影響。',
    '幸運:状態異常の回避率に影響。平均値が宝箱のレア率に影響。\n採取の成果判定や一部イベントの判定に影響。'
  ];
  const yesNo = ['はい', 'いいえ'];
  const confirmText = "にポイントを振ります。\n振りなおせませんがよろしいですか？";
  ...
  Window_LevelUp.prototype.drawAllItems = function () {
    ...
    this.drawText('残りポイント(' + this._actor.levelPoint + ')', this.itemPadding() + this.colSpacing() / 2, this.itemHeight() * 2 + this.rowSpacing() / 2, this.innerWidth);


    this.drawText('現在のステータス', this.itemPadding() + this.colSpacing() / 2, this.itemHeight() * 7 + this.rowSpacing() / 2, this.innerWidth);
